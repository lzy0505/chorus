...
- [ ] `services/gitbutler.py` - GitButler CLI integration
  ...
  - [ ] `mark_stack(stack)` - Mark stack for auto-assign via `but mark`
  - [ ] `unmark()` - Remove mark via `but unmark`
...
- `but unmark` - Remove mark (before marking another stack)
...
1. **Marking**: When a stack is marked, all new unassigned changes go to it
2. **File Ownership**: Once a file is in a stack, subsequent changes stay there

**Task Lifecycle:**
```
Start:  but branch new → but mark → start Claude
Switch: but unmark → but mark (new task)
Complete: but unmark → kill tmux
Fail:   but unmark → optionally but branch delete → kill tmux
```

GitButler auto-commits via Claude Code hooks:
- `but claude pre-tool` - Before file edits (registers session)
- `but claude post-tool` - After file edits (assigns & commits changes)
- `but claude stop` - When Claude stops (finalizes commits)

**Note:** Commit messages are auto-generated by GitButler based on session context.
