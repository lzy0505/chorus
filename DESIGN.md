...
Start the task — creates stack, marks for auto-assign, spawns tmux, launches Claude.

**Implementation:**
1. Create GitButler stack: `but branch new task-{id}-{slug} -j`
2. Mark stack for auto-assignment: `but mark task-{id}-{slug}`
...
7. Update task: `status = running`, `started_at = now()`, `tmux_session = task-{id}`, `stack_id = {cli_id}`
...
Complete the task — unmarks stack, ensures changes are committed, kills tmux.

**Implementation:**
1. Unmark the stack: `but unmark` (allows other tasks to be marked)
2. GitButler auto-commits via native hooks (already handled by `but claude stop`)
3. Optionally verify stack has commits: `but branch show {stack} -j`
4. Kill tmux session
5. Update task: `status = completed`, `completed_at = now()`
...
**Note:** Commit messages are generated by GitButler based on the Claude session context. File ownership persists after unmarking, so any subsequent changes to files in this stack will still be assigned to it.

#### `POST /api/tasks/{task_id}/fail`
Mark task as failed — unmarks stack, optionally deletes it, kills tmux.
...
**Implementation:**
1. Unmark the stack: `but unmark`
2. If `discard_stack`: Delete stack: `but branch delete {stack} --force`
3. Kill tmux session
4. Update task: `status = failed`, `result = {reason}`
...
GitButler automatically assigns file changes to stacks using two mechanisms:

1. **Marking (`but mark`)**: When a stack is marked, ALL new unassigned changes go to that stack. Only one stack can be marked at a time.

2. **File Ownership**: Once a file has been modified in a stack, subsequent changes to that file stay in that stack — even across Claude session restarts.

**Task Lifecycle with Auto-Assignment:**
```
Task Start:
  1. but branch new task-{id}-{slug}    # Create stack
  2. but mark task-{id}-{slug}          # Mark for auto-assign
  3. Start Claude in tmux               # Changes go to marked stack

Task Switch (concurrent tasks):
  1. but unmark                         # Remove current mark
  2. but mark task-{other-id}-{slug}    # Mark other task's stack
  Note: File ownership ensures existing files stay with their stacks

Task Complete:
  1. but unmark                         # Remove mark
  2. Kill tmux session                  # Cleanup
```

**Auto-Commit via Claude Code Hooks:**
GitButler provides native hooks that automatically commit changes:
- `but claude pre-tool` — Called before file edits, registers session
- `but claude post-tool` — Called after file edits, assigns & commits changes
